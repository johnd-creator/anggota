      ...
      <div class="lg:col-span-3">
        <CardContainer padding="lg" shadow="sm">
          <div class="flex items-start justify-between">
            <div>
              <h3 class="text-lg font-semibold text-neutral-900">Mutasi per Unit</h3>
              <p class="text-sm text-neutral-600">Ringkasan KPI</p>
            </div>
            <div class="flex items-center gap-3">
              <Badge variant="brand">Total: {{ kpi.total }}</Badge>
              <Badge :variant="slaBreaches? 'warning' : 'neutral'">SLA Breach: {{ slaBreaches }}</Badge>
            </div>
          </div>
            ...
            <table class="min-w-full divide-y divide-neutral-200">
              <thead class="bg-neutral-50 sticky top-0 z-10">
                <tr>
                  ...
                  <th class="px-4 py-2 text-left text-xs font-medium text-neutral-500">Asal → Tujuan</th>
              ...
              <tbody class="divide-y divide-neutral-200 bg-white">
                <tr v-for="(r,i) in rows" :key="i">
                  ...
                  <td class="px-4 py-2 text-sm">{{ r.from }} → {{ r.to }}</td>
                ...
                <tr v-if="expanded[i]" :key="`exp-${i}`">
                  <td colspan="4" class="px-4 py-3 bg-neutral-50">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                      ...
                      <div>
                        <div class="text-xs text-neutral-600">Dokumen</div>
                        <a v-if="r.document" :href="r.document" class="text-brand-primary-600 text-sm">Lihat Dokumen</a>
                      </div>
                    </div>
                  </td>
                </tr>
...
const rows = ref([
  { member:'Andi', from:'Unit A', to:'Unit B', status:'pending', reason:'Rotasi', document:'#' },
]);
const expanded = ref({});
function toggle(i){ expanded.value[i] = !expanded.value[i]; }
const kpi = reactive({ total: 10 });
const slaBreaches = 0;
const lastUpdated = new Date().toLocaleDateString('id-ID');
function apply(){ }

const exportOpen = ref(false);
const exportForm = reactive({ format: 'CSV' });
