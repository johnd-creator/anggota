<template>
  <AppLayout pageTitle="Reports: Iuran (CSV)">
    <div class="space-y-6">
      <!-- Header -->
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 class="text-xl font-bold text-neutral-900">Export Iuran</h2>
          <p class="text-sm text-neutral-500">Download data pembayaran iuran dalam format CSV.</p>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <CardContainer class="lg:col-span-2">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <!-- Period (Month) -->
            <div>
               <InputField label="Periode (Bulan)" type="month" v-model="form.period" />
               <p class="text-xs text-neutral-500 mt-1">Kosongkan untuk semua periode</p>
            </div>
            ...
            <div>
               <InputField label="Tanggal Transaksi (Dari)" type="date" v-model="form.date_start" />
            </div>
            <div>
               <InputField label="Tanggal Transaksi (Sampai)" type="date" v-model="form.date_end" />
            </div>

            <!-- Options -->
            <div class="md:col-span-2 flex items-center gap-2 mt-2">
               <Checkbox v-model:checked="form.include_notes" id="include_notes" />
               <label for="include_notes" class="text-sm text-neutral-700">Sertakan Catatan (Notes)</label>
            </div>
          </div>
        </CardContainer>
...
const amountOptions = [
    { value: '', label: 'Semua' },
    { value: 'default', label: 'Sesuai Default' },
    { value: 'custom', label: 'Custom (Beda Nominal)' },
];
...
function downloadCsv() {
   ...
   if (form.value.period) params.append('period', form.value.period);
   if (form.value.amount_default) params.append('amount_default', form.value.amount_default);
   ...
   if (form.value.include_notes) params.append('include_notes', '1');
